...
func (a *Adapter) RemovePolicy(sec string, ptype string, rule []string) error {
	line := a.savePolicyLine(ptype, rule)
	err := a.rawDelete(a.db, line) //can't use db.Delete as we're not using primary key https://web.archive.org/web/20200509020554/https://jinzhu.me/gorm/crud.html#update
	return err
}
...
func (a *Adapter) RemovePolicies(sec string, ptype string, rules [][]string) error {
	return a.db.Transaction(func(tx *gorm.DB) error {
		for _, rule := range rules {
			line := a.savePolicyLine(ptype, rule)
			if err := a.rawDelete(tx, line); err != nil { //can't use db.Delete as we're not using primary key https://web.archive.org/web/20200509020554/https://jinzhu.me/gorm/crud.html#update
				return err
			}
		}
		return nil
