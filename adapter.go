...
func (a *Adapter) UpdateFilteredPolicies(sec string, ptype string, newPolicies [][]string, fieldIndex int, fieldValues ...string) ([][]string, error) {
	...
	for i := range newP {
		if err := tx.Where(line.queryString()).Delete(&oldP).Error; err != nil {
			tx.Rollback()
			return nil, err
		}
		if err := tx.Model(&CasbinRule{}).Create(newP[i]).Error; err != nil {
			tx.Rollback()
			return nil, err
		}
	}
...
func (c *CasbinRule) queryString() interface{} {
